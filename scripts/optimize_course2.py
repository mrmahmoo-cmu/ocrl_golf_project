...
CMA-ES optimization for Course 2: Gentle Dogleg with wind.
Includes landed-terrain penalties and crosswind.
...
SAND_PENALTY = 10.0
ROUGH_PENALTY = 25.0
LANDED_ROUGH_PENALTY = 50.0
LANDED_SAND_PENALTY = 30.0
...
def simulate(aim_angle, elbow_init, torque_profile):
    ...
    return info["dist_to_hole"], {
        ...
        "terrain": info["terrain"], "landed_terrain": info["landed_terrain"],
    }
...
def run_cmaes(x0, sigma0, seed, max_evals, label=""):
    ...
    opts = {
        ...
        'tolfun': 1e-5, 'verb_disp': 0, 'verb_log': 0, 'seed': seed,
    }
...
def main():
    print("=" * 60)
    print("COURSE 2: Gentle Dogleg — CMA-ES with Restarts")
    print("=" * 60)
    ...
    env_final = GolfSwingEnv(
        ...
        terrain_zones=TERRAIN_ZONES, model_path=MODEL_PATH, wind=WIND,
    )
    obs = env_final.reset(elbow_init=best_elbow, wrist_init=0.0, aim_init=best_aim)
    trajectory = []
    for step in range(env_final.max_steps):
        obs, info = env_final.step(get_torque_at_time(best_torques, step * env_final.ctrl_dt))
    ...
    print(f"\n  Aim angle:       {np.degrees(best_aim):.1f} deg")
    print(f"  Backswing angle: {np.degrees(best_elbow):.1f} deg")
    print(f"  Wind:            ({WIND[0]}, {WIND[1]}, {WIND[2]}) m/s")
    print(f"  Torque profile:")
    for i in range(N_BINS):
        ts, te = i * BIN_DT, (i + 1) * BIN_DT
        print(f"    [{ts:.3f}-{te:.3f}s]  elbow={best_torques[i,0]:+7.1f} Nm  wrist={best_torques[i,1]:+6.1f} Nm")
    print(f"\n  Ball final:      ({obs['ball_pos'][0]:.2f}, {obs['ball_pos'][1]:.2f}, {obs['ball_pos'][2]:.2f})")
    ...
    np.savez(
        ...
        model_path=str(MODEL_PATH), has_aim=True, wind=WIND,
    )
    print(f"\n  Saved to: {save_path}")
